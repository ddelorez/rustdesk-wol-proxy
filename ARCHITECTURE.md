...
The RustDesk Auto-WOL project solves the inability for offsite users to wake
sleeping machines in a self-hosted RustDesk environment. The solution
architecture consists of three integrated components:

1. **WOL Proxy API** (Phase 1): A lightweight Flask/gunicorn service running on
   the RustDesk server that accepts wake requests and sends magic packets to
   targets
2. **Custom RustDesk Client** (Phase 2): A forked RustDesk client that detects
   offline peers and automatically invokes WOL
3. **Configuration & Orchestration** (Ongoing): Secure delivery of API keys,
   ID↔MAC mappings, and deployment automation

**Key Innovation:** By centralizing WOL on the RustDesk server (which sits on
the target LAN), we enable offsite clients to wake machines without requiring
internet-accessible broadcast protocols.
...
RustDesk's built-in Wake-on-LAN feature operates only within local LANs. When
attempting WOL from an offsite client:

- **Issue**: Magic packets (UDP broadcast to port 9) cannot route from internet
  to LAN
...
- **Network topology**: Single LAN (10.10.10.0/24), RustDesk server at
  10.10.10.145
- **User base**: Mix of LAN and offsite clients connecting to shared RustDesk
  server
- **Scope**: Target 5-20 machines initially; expandable to larger deployments
- **Security**: API must not expose machine information or enable unauthorized
  wakeups
...
- **Responsibility**: Detect offline connections, invoke WOL API, manage retry
  logic
- **Interface**: Configuration panel (API URL, API key), automatic detection
  logic
...
| --- | --- | --- | --- |
| Client → API | Outbound | HTTPS/HTTP | WOL request with creds |
...
- Log File: `/var/log/rustdesk-wol-proxy.log` (rotating, 5MB max per file, 3
  backups)
...
```text
2026-02-10 14:23:45,123 INFO: WOL packet sent to AA:BB:CC:DD:EE:FF
(ID: 123456789) [in app.py:55]
2026-02-10 14:24:12,456 WARNING: Invalid API key attempt for ID 987654321
[in app.py:45]
2026-02-10 14:25:01,789 ERROR: Failed to send WOL packet to
11:22:33:44:55:66: [Errno 1] Operation not permitted [in app.py:62]
```
...
| --- | --- | --- | --- |
| 400 | `MISSING_PARAMETER` | Missing `id` or `key` | Missing id/key |
| 403 | `INVALID_KEY` | API key mismatch | Invalid API key |
| 404 | `UNKNOWN_ID` | ID not in mapping | No MAC registered |
| 500 | `SEND_FAILED` | Send error | Failed to send packet |
...
- HMAC-SHA256 signing: Adds complexity without significant gain for internal
  API
- JWT tokens: Better for multi-tenant; overkill for single organization
- mTLS certificates: Would require client certificate distribution (future
  enhancement)
...
- Threat model: Prevent unauthorized WOL invocation, not protect packet
  contents
...
| Threat | Attack | Impact | Phase 1 | Future |
| --- | --- | --- | --- | --- |
| Unauth WOL | Wrong API key | Wrong machine wake | Key+logging | Rate limit |
| DoS Flooding | Repeat requests | Broadcast spam | Logging | Rate limit |
| Key Leakage | Binary compromise | Full API access | Rotation | Key version |
| Info Disclosure | ID/MAC enum | Topology revealed | Reject silent | 404 only |
| Network MITM | HTTP intercept | Key/ID exposed | LAN only | TLS |
| Privilege Esc | Root access | System compromise | Dedicated user | SELinux |
...
sudo chown ubuntu:ubuntu /opt/rustdesk-wol-proxy
```
...
- [ ] Move ALLOWED_IDS to configurable source (start with comment-based
  config)
...
- [ ] Implement log rotation via RotatingFileHandler (already done in current
  code)
...
- [ ] Verify log output to `/var/log/rustdesk-wol-proxy.log` (create dev log
  path)
- [ ] Test concurrent requests (stress/load test): `ab -n 100 -c 10
  "http://localhost:5001/wake?id=123&key=testkey"`
...
- [ ] Test HTTP request from offsite: `curl
  "http://10.10.10.145:5001/wake?id=<TEST_ID>&key=<API_KEY>"`
...
- [ ] Call WOL API: `curl
  "http://10.10.10.145:5001/wake?id=<ID>&key=<KEY>"`
- [ ] Monitor target machine for power LED / wake indicators
- [ ] Confirm RustDesk service starts on target
- [ ] Verify broadcast packet seen in network capture (optional tcpdump
  verification)
...
*Phase 2 is OUT OF SCOPE for this architecture document but described for
context.*
...
1. **Connection State Detection**: Hook into RustDesk connection logic to detect
   "peer offline" state
...
4. **Retry Logic**: Wait N seconds for boot → attempt reconnection → repeat up
   to M times
...
```yaml
wol_enabled: true
wol_api_url: "https://rustdesk-wol.internal.net/wake"
...
- Key should be stored in OS keyring (Windows Credential Manager, macOS
...
| --- | --- | --- | --- |
| Magic packet missed | Medium | High | Verify MAC, test with tcpdump |
| Key in logs | Low | Critical | Key masking + filtering |
| Service crash | Medium | High | Watchdog, systemd restart |
| Race condition | Low | Low | gunicorn handles concurrency |
| Broadcast storms | Low | Medium | Rate limit per IP |
| Wrong MAC | Medium | Medium | Pre-validate + confirm |

### Operational Risks

| Risk | Prob. | Impact | Mitigation |
| --- | --- | --- | --- |
| Key compromise | Medium | Critical | Rotate + log + rate limit |
| Unauth WOL | High | Low | Explicit action + dialog |
| Disk full | Low | Medium | Auto rotation + archive |
| Config change | Low | High | Document + version |
| Power exceeded | Low | Medium | Stagger + queue |

### Security Risks

| Risk | Prob. | Impact | Mitigation |
| --- | --- | --- | --- |
| DoS requests | High | Medium | Rate limit + IP blocking |
| Key in Git | High | Critical | Use .env + .gitignore |
| Info disclose | Medium | Medium | Reject silently |
| MITM HTTP | Medium | High | Use HTTPS proxy |
| Priv. escalate | Low | Critical | Non-root + hardening |

### Dependency Risks

| Dependency | Version | Risk | Mitigation |
| --- | --- | --- | --- |
| Flask | 3.0.3 | Security vulns | Monitor + update quarterly |
| wakeonlan | 3.1.0 | Abandoned? | Backup implementation |
| gunicorn | 23.0.0 | CVE exposure | Monitor + patch |
| Python | 3.8+ | EOL risk | Target 3.10+ |
...
**Phase 1 Success Criteria**: API operational, tested, and monitored, ready for
client integration
...
```bash
curl -X GET "http://10.10.10.145:5001/wake?id=123456789&key=wol_prod"
```

**With HTTPS (Phase 2+)**:

```bash
curl -X GET "https://rustdesk-wol.internal.net/wake?id=123456789"
```
...
params = {"id": "123456789", "key": "wol_prod_xxxxx"}
...
| Term | Definition |
| --- | --- |
| **WOL** | Wake-on-LAN technology for remote power on |
| **Magic Packet** | Special UDP packet that wakes WOL machines |
| **Broadcast IP** | Network broadcast address (10.10.10.255) |
| **WSGI** | Web Server Gateway Interface for Python |
| **gunicorn** | Python WSGI HTTP server |
| **systemd** | Linux service manager |
| **API Key** | Shared authentication secret |
| **RustDesk ID** | Global client identifier |
| **MAC Address** | Hardware identifier (AA:BB:CC:DD:EE:FF) |
| **Reverse Proxy** | Nginx/intermediate server |
